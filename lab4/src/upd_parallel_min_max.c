...
volatile sig_atomic_t timeout_expired = 0; // Глобальная переменная для отслеживания таймаута
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < pnum; i++) {
        ...
        if (child_pid >= 0) {
            ...
            if (child_pid == 0) {
                // Добавляем обработку сигнала SIGALRM в дочернем процессе
    ...
    // Установка таймера SIGALRM для завершения процессов по истечению таймаута
    if (timeout > 0) {
        signal(SIGALRM, alarm_handler);
        alarm(timeout);

        while (timeout_expired == 0) {
            usleep(10000);
        }
